...
impl AsyncVerifier {
    ...
    pub fn new(config: Config) -> Result<AsyncVerifier> {
        let client = if !config.proxy_url.is_empty() && config.proxy_username.is_empty() {
            AsyncVerifier::get_client_proxy(&config)?
        } else if !config.proxy_url.is_empty() && !config.proxy_username.is_empty() {
            AsyncVerifier::get_client_proxy_with_auth(&config)?
        } else {
    ...
    /// # Errors
    /// Will return `Err` of type `YubicoError::MultipleErrors` if anything during the verification fails
    /// The `Err` could contain multiple errors, the errors are withing a `Vec<YubicoError>`
